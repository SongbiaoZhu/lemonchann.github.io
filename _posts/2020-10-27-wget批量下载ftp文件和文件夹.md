...
因为经常做蛋白质组学数据分析时，最经常使用的数据库就是uniprot数据库，但是uniprot accession经常需要进行ID 转化，所以这里下载了Uniprot中提供的ftp IDmapping/byorgnism的数据，是ftp页面，所以想批量下载所提供物种的id转换数据。应该如何使用wget操作呢？

<!-- more -->

## 简介

Ftp是一种常见的在线数据库，今天介绍一种可以批量下载文件夹的方法，也是shell中常常用到的wget命令：
...
这里介绍一个例子，我们想要下载FTP服务器的一个文件夹而不需要其他文件夹的文件，这个时候命令可以这样写：
...
因为经常做蛋白质组学数据分析时，最经常使用的数据库就是uniprot数据库，但是uniprot accession经常需要进行ID 转化，所以这里下载了Uniprot中提供的ftp IDmapping/byorgnism的数据，是ftp页面，所以想批量下载所提供物种的id转换数据。

从[html](ftp://ftp.uniprot.org/pub/databases/uniprot/current_release/knowledgebase/idmapping/by_organism/)中可以看到所下载的数据的修改时间是 2019.9.18.

我简单地修改了示例的代码中的网页地址，即成功地开始下载了：

```shell
wget -r -c -nH -np ftp://ftp.uniprot.org/pub/databases/uniprot/current_release/knowledgebase/idmapping/by_organism/
```

```shell
FINISHED --2019-09-20 10:17:34--
Downloaded: 25 files, 259M in 25m 34s (173 KB/s)

```

如上，耗时25min，下载了25个文件，所下载的文件在当前工作目录中，并且目录包含了原始ftp页面的文件夹目录结构。

最后，为了方便使用，我将所有的压缩文件拷贝至一个新的文件夹`uniprotID`中，然后使用shell命令批量解压缩。
...
注意看到，解压后自动使用了解压出来的文件替换了原来的压缩文件，即原来的压缩文件已经被删除。

## uniport ID的API使用说明

uniprot提供页面解释了所有可以转换的ID的分类、名称以及其对应的缩写，是一个表格，可以查看，[Programmatic access - Mapping database identifiers](https://www.uniprot.org/help/api_idmapping)。

我实践了python3的使用方法，并进行了修改，以方便于对txt形式的批量accession ID进行转换，并且保存到转换后的txt文档中，脚本为`uniprotIDmapApi.py`

## 致谢

* [ 利用wget命令批量下载ftp文件和文件夹](https://www.cnblogs.com/muuyouzhi/p/9524377.html)

* [Extract and delete all .gz in a directory- Linux](https://stackoverflow.com/questions/16038087/extract-and-delete-all-gz-in-a-directory-linux)